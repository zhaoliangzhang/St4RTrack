...
class Attention(nn.Module):
    ...
    def __init__(self, dim, rope=None, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., zero_init=False):
        super().__init__()
        ...
        # Add zero-initialized gate if zero_init is True
        self.zero_init = zero_init
        if zero_init:
            self.gate = nn.Parameter(torch.zeros(1))
            # Zero initialize the projection layer weights and bias
    ...
    def forward(self, x, xpos=None):
        ...
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).transpose(1,3) #[B, num_heads,3, num_tokens, head_dim]
        q, k, v = [qkv[:,:,i] for i in range(3)] 

        if self.rope is not None:
            # Modify head dimension to be divisible by 3 by padding
            head_dim = q.size(-1)
            pos_dim = xpos.size(-1)
            pad_size = (-head_dim) % pos_dim  # Calculate padding needed to make divisible by 3
            if pad_size > 0:
                ...
                k = self.rope(k, xpos)
                # Remove padding
                q = q[..., :head_dim].contiguous()
                k = k[..., :head_dim].contiguous()
            else:
                # If already divisible by 3, apply RoPE directly
                q = self.rope(q, xpos)
                k = self.rope(k, xpos)

        if self.zero_init:
            with torch.backends.cuda.sdp_kernel(enable_flash=True):
                x = self.gate * pytorch_attn(q, k, v, scale=self.scale).transpose(1, 2).reshape(B, N, C)
                x = self.proj(x)
                x = self.proj_drop(x)
        else:
            with torch.backends.cuda.sdp_kernel(enable_flash=True):
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, norm_mem=True, rope=None, arch_mode='VanillaDust3r', rope_mode='full_3d'):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, rope=rope, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, y, xpos, ypos):
        infer_mode = 'train' if self.training else 'eval'
        x = x + self.drop_path(self.attn(self.norm1(x), xpos))
        y_ = self.norm_y(y)
...
class PositionGetter3D(object):
    ...
    def __call__(self, b, t, h, w, device):
        if not (b,t,h,w) in self.cache_positions:
            ...
            self.cache_positions[t, h, w] = torch.cartesian_prod(z, y, x)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True, init='xavier'):
        ...
        self.position_getter3d = PositionGetter3D()
        self.init_type = init
