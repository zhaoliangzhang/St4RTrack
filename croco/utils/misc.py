...
class MetricLogger(object):
    ...
    def synchronize_between_processes(self):
        for meter_key, meter in self.meters.items():
    ...
    def log_every(self, iterable, print_freq, header=None, max_iter=None):
        ...
        print('rank{} {} Total time: {} ({:.4f} s / it)'.format( get_rank(),
            header, total_time_str, total_time / len_iterable), force=True)
...
def get_parameter_groups(model, weight_decay, layer_decay=1.0, skip_list=(), no_lr_scale_list=[], zero_lr_scale_list=None):
    ...
    for name, param in model.named_parameters():
        ...
        # Handle custom lr_scale attribute
        lr_scale = getattr(param, 'lr_scale', 1.0)
        
        # Set lr_scale to 0.0 for specified parameters, to achieve no weight update but still keep the gradient back propagation.
        if zero_lr_scale_list is not None:
            assert len(zero_lr_scale_list) > 0
            if any(name.startswith(prefix) for prefix in zero_lr_scale_list):
                lr_scale = 0.0
            
        if lr_scale != 1.0:
            group_name = f'{group_name}_lr_scale_{lr_scale}'

        if group_name not in parameter_group_names:
            if not skip_scale:
                scale = layer_decay_values[layer_id] * lr_scale  # Combine layer decay with custom lr_scale
            else:
                scale = lr_scale  # Just use custom lr_scale
