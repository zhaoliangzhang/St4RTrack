...
class BasePCOptimizer (nn.Module):
    ...
    def save_depth_maps(self, path):
        ...
        for i, depth_map in enumerate(depth_maps):
            ...
            img_path = f'{path}/frame_{(i+1):04d}.png'
            cv2.imwrite(img_path, depth_map_colored)
            images.append(Image.open(img_path))
            np.save(f'{path}/frame_{(i+1):04d}.npy', depth_map.detach().cpu().numpy())
...
def global_alignment_iter(net, cur_iter, niter, lr_base, lr_min, optimizer, schedule, temporal_smoothing_weight=0):
    ...
    if net.partial_init:  # zero the gradient of the fixed parameters
        grad_mask = ~net.msk
        net.im_depthmaps.grad *= grad_mask.unsqueeze(-1)
        net.im_focals.grad *= grad_mask.unsqueeze(-1)
        net.im_poses.grad *= grad_mask.unsqueeze(-1)
