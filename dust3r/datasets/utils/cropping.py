...
def rescale_image_depthmap_crop(image, depthmap, camera_intrinsics=None, output_resolution=None, force=True, random_crop=False, random_seed=42):
    """ 
    Jointly rescale a (image, depthmap) so that the long side remains unchanged, 
    and the short side is cropped symmetrically to match the output resolution.
    """
    # Convert image to ImageList
    ...
    output_resolution_resize = np.floor(input_resolution * scale_final).astype(int)

    # first rescale the image so that it contains the crop
    image = image.resize(tuple(output_resolution_resize), resample=lanczos if scale_final < 1 else bicubic)
    if depthmap is not None:
        depthmap = cv2.resize(depthmap, output_resolution_resize, fx=scale_final,
                              fy=scale_final, interpolation=cv2.INTER_NEAREST)

    # crop the image
    h_need_to_crop = output_resolution_resize[1] - output_resolution[1]
    w_need_to_crop = output_resolution_resize[0] - output_resolution[0]
    assert h_need_to_crop >= 0 and w_need_to_crop >= 0
    ...
    if not random_crop:
        ...
        if w_need_to_crop > 0:
            l = np.random.randint(0, w_need_to_crop)
        else:
            l = 0
        if h_need_to_crop > 0:
            t = np.random.randint(0, h_need_to_crop)
        else:
            t = 0
        r = l + output_resolution[0]
        b = t + output_resolution[1]
    ...
    return image.to_pil(), depthmap, camera_intrinsics, (l, t, r, b)
...
def camera_matrix_of_crop(input_camera_matrix, input_resolution, output_resolution, scaling=1, offset_factor=0.5, offset=None):
    ...
    # print(output_camera_matrix_colmap.shape)

    if output_camera_matrix_colmap.ndim == 2:
        output_camera_matrix_colmap[:2, :] *= scaling
        output_camera_matrix_colmap[0, 2] -= offset[0]
        output_camera_matrix_colmap[1, 2] -= offset[1]

    elif output_camera_matrix_colmap.ndim == 3:
        output_camera_matrix_colmap[:, :2, :] *= scaling
        output_camera_matrix_colmap[:, 0, 2] -= offset[0]
        output_camera_matrix_colmap[:, 1, 2] -= offset[1]
