...
class PatchEmbedDust3R(PatchEmbed):
    def forward(self, x, **kw):
        ...
        x = self.proj(x)
        # pos = self.position_getter3d(B, x.size(2), x.size(3), x.device)
        pos = self.position_getter3d(1, B, x.size(2), x.size(3), x.device).reshape(B, -1, 3)
...
class ManyAR_PatchEmbed (PatchEmbed):
    ...
    def forward(self, img, true_shape, seq_id=None):
        B, C, H, W = img.shape
        ...
        pos = self.position_getter3d(1, B, H, W, x.device).reshape(B, -1, 3)
