...
def screenspace_ptc_depth_to_camera_coordinates(ptc, camera_intrinsics, pseudo_focal=None):
    """
    Args:
        - screenspace ptc (Nx3 array):
        - camera_intrinsics: a 3x3 matrix
    Returns:
        pointmap of absolute coordinates (HxWx3 array), and a mask specifying valid pixels.
    """
...
def depthmap_to_absolute_camera_coordinates(depthmap, camera_intrinsics, camera_pose, z_far=0, proj_mode='depth', **kw):
    """
    Args:
        - depthmap (HxW array):
        - camera_intrinsics: a 3x3 matrix
    ...
    Returns:
        pointmap of absolute coordinates (HxWx3 array), and a mask specifying valid pixels."""
    if proj_mode == 'ptc':
        X_cam, valid_mask = screenspace_ptc_depth_to_camera_coordinates(depthmap, camera_intrinsics)
    else:
    ...
    if camera_pose is not None:
        ...
        if proj_mode == 'ptc':
            X_world = geotrf(camera_pose, X_cam, ncol=3)
            # X_world_1 = np.einsum("ik, vk -> vi", R_cam2world, X_cam) + t_cam2world[None, :]
        else:
...
def normalize_pointcloud_seq(pts1, pts2, norm_mode='avg_dis',  traj_mask=None, valid1=None, valid2=None, ret_factor=False):
    """ renorm pointmaps pts1_seq with pts2_seq's scale with norm_mode
    """
    assert pts1.ndim >= 3 and pts1.shape[-1] == 3
    assert pts2 is None or (pts2.ndim >= 3 and pts2.shape[-1] == 3)
    norm_mode, dis_mode = norm_mode.split('_')
    ...
    if norm_mode == 'avg':
        # normalize according to pts2's scale
        nan_pts2, nnz2 = invalid_to_zeros(pts2, valid2, ndim=3) if pts2 is not None else (None, 0)
        all_pts = nan_pts2 if pts2 is not None else nan_pts1

        # compute distance to origin
        all_dis = all_pts.norm(dim=-1)
        if dis_mode.startswith('dis'):
            pass  # do nothing
        elif dis_mode == 'log1p':
            all_dis = torch.log1p(all_dis)
        elif dis_mode == 'warp-log1p':
        ...
        if dis_mode == 'dis-seq':
            norm_factor = (all_dis.sum(dim=1).sum() / (nnz2.sum() + 1e-8)).unsqueeze(0).repeat(len(pts1)) # weighted avg of the norm factor of all frames
        else:
            norm_factor = all_dis.sum(dim=1) / (nnz2 + 1e-8)
    else:
        # gather all points together (joint normalization)
        nan_pts1 = invalid_to_nans(pts1, valid1, ndim=3)
        nan_pts2 = invalid_to_nans(pts2, valid2, ndim=3) if pts2 is not None else None
